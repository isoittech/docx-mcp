...
| `word_start_document` | sync | destructive | metadata/layout/design/expected sections/template | draft | user+conversation / 1h | 1〜50 sections | add |
...
| `word_finish_document` | async | destructive | completed draft | draft -> job | user+conversation | section 数一致必須 | wait、全 preview |
...
| `word_wait_for_job` | sync wait | RO, idempotent | job, 1〜50秒 | job | user+conversation | terminal または timeout snapshot | 成功なら全 preview |
...
共通 terminal state は `succeeded`、`failed`、`canceled`、`timed_out`、`rejected_unsafe_document`。入力エラーは `status=invalid_input`、`code`、`field_path`、`message`、`correction` を返す。queue full は `resource_exhausted`、scope 不一致は存在を漏らさず `not_found` とする。
