...
public static class WordServerInstructions
{
    public static string Build(WordMcpOptions options)
    {
        ...
        return $$"""
            ...
            For an existing document: call word_analyze, wait with word_wait_for_job, obtain only needed chunks with word_get_analysis_chunk, then use returned targets with word_replace_text or word_apply_edits. Unsafe documents end as rejected_unsafe_document and expose no body text or targets.
            ...
            Do not send user IDs, conversation IDs, message IDs, local paths, output paths, artifact URLs, arbitrary URLs, base64, XML, HTML, Markdown, CSS, code, shell commands, or field instructions to any tool. Images must use opaque same-user PNG/JPEG image_file_id values and require alt text.
            {{notice}}
            """;
    }
}
